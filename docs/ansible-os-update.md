...
node. Every apt-touching call in the role therefore waits for the lock
(`os_update_dpkg_lock_timeout`, default 300s) and retries — `apt.yml` always did,
`holds.yml` did not until it cost a run.
